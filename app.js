...
function removeDuplicates() {
    ...
    services.forEach(item => {
        const creditorPart = item.creditor ? `-${item.creditor.toLowerCase().trim()}` : '';
...
function setupEventListeners() {
    ...
    itemNameInput.addEventListener('input', (e) => {
        if (itemTypeInput.value !== 'service' && itemTypeInput.value !== 'overdue') return;
        ...
        if (name.includes('luz') || name.includes('gas') || name.includes('energia')) {
            showDynamic = true;
...
function handleFormSubmit(e) {
    ...
    if (type === 'service' || type === 'overdue') {
        ...
        itemData.consumptionMonthEnd = consMonthEndSelect.value ? parseInt(consMonthEndSelect.value) : null;
        
        if (consUnitInput.value) itemData.consumptionUnit = parseFloat(consUnitInput.value);
        if (nextVisitInput.value) itemData.nextMeasurementDate = parseInt(nextVisitInput.value);
        if (internetCloseInput.value) itemData.billingCloseDate = parseInt(internetCloseInput.value);
        
    } else if (type === 'loan') {
        itemData.creditor = document.getElementById('loan-creditor').value.trim();
        itemData.currentInstallment = parseInt(document.getElementById('loan-current-installment').value) || 1;
        itemData.totalInstallments = parseInt(document.getElementById('loan-total-installments').value) || 1;
    ...
    if (id) {
        ...
        if (index !== -1) {
            ...
            if (services[index].paymentDate) itemData.paymentDate = services[index].paymentDate;
...
window.editItem = function(id) {
    ...
    if (item.type === 'service' || item.type === 'overdue') {
        ...
        consMonthEndSelect.value = item.consumptionMonthEnd !== null ? item.consumptionMonthEnd : "";
        
        itemNameInput.dispatchEvent(new Event('input'));
        
        if (item.consumptionUnit) consUnitInput.value = item.consumptionUnit;
        if (item.nextMeasurementDate) nextVisitInput.value = item.nextMeasurementDate;
        ...
        document.getElementById('loan-current-installment').value = item.currentInstallment || 1;
        document.getElementById('loan-total-installments').value = item.totalInstallments || 1;
