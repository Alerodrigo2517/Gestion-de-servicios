    ...
    <div className="glass-premium rounded-2xl p-5 shadow-2xl relative overflow-hidden transition-all duration-300 hover:border-white/10 w-full animate-slide-up">
      ...
      <div className="grid grid-cols-7 gap-1.5 text-center">
        {gridSlots.map((day, idx) => {
          ...
          // Day cell class
          let cellClass =
            'h-9 flex flex-col items-center justify-center rounded-lg text-xs transition-all duration-200 select-none cursor-pointer border ';

          if (isSelected) {
            cellClass +=
              'bg-sky-500/20 text-sky-400 border-sky-500/40 font-bold shadow-[0_0_10px_rgba(14,165,233,0.15)]';
          } else {
            cellClass +=
            ...
            >
              ...
              <div className="h-3 flex items-center justify-center mt-0.5">
                {dayServices.length > 0 && (
                  <span
                    className={`text-[8px] font-extrabold px-1 rounded-md scale-95 ${
                      hasHigh
                        ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30 shadow-[0_0_8px_rgba(244,63,94,0.15)] animate-pulse'
                        : hasMedium
