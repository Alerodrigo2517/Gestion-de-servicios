...
export function getServiceDueDate(service) {
  ...
  const month = service.paymentMonth !== undefined ? service.paymentMonth : new Date().getMonth();
...
export function getServiceStatus(service) {
  ...
  if (daysRemaining < 0) {
    return {
      status: 'VENCIDO',
      daysRemaining,
      colorClass: 'text-rose-400 bg-rose-500/10 border-rose-500/20 shadow-[0_0_15px_rgba(244,63,94,0.15)] animate-pulse',
      badgeText: 'Vencido',
      severity: 'high'
    };
    ...
    return {
      status: 'VENCE_HOY',
      daysRemaining,
      colorClass: 'text-orange-400 bg-orange-500/10 border-orange-500/20 shadow-[0_0_15px_rgba(249,115,22,0.15)] animate-pulse',
      badgeText: 'Vence hoy',
      severity: 'high'
    };
